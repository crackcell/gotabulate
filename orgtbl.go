...
func (this *OrgtblFormatter) Format(info *TableInfo) string {
	str := ""
	var header []string
...
func (this *OrgtblFormatter) formatLine(row []string, info *TableInfo) string {
	str := ""
	l := len(row)
...
func (this *OrgtblFormatter) formatHeaderSep(info *TableInfo) string {
	str := fmt.Sprintf("|%s", strings.Repeat("-", info.CellWidth[0]+2))
	for i := 1; i < info.ColumnSize; i++ {
